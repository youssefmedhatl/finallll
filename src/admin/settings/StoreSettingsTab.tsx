...
function StoreSection({ canWrite }: { canWrite: boolean }) {
  ...
  const [form, setForm] = useState<StoreValue | null>(null)

  useEffect(() => {
    if (row) setForm(row.value as unknown as StoreValue)
  }, [row])
...
function LoyaltySection({ canWrite }: { canWrite: boolean }) {
  ...
  const [form, setForm] = useState<LoyaltyValue | null>(null)

  useEffect(() => {
    if (row) setForm(row.value as unknown as LoyaltyValue)
  }, [row])
...
function ShippingSection({ canWrite }: { canWrite: boolean }) {
  ...
  const [form, setForm] = useState<ShippingValue | null>(null)

  useEffect(() => {
    if (row) setForm(row.value as unknown as ShippingValue)
  }, [row])
...
function TaxSection({ canWrite }: { canWrite: boolean }) {
  ...
  const [form, setForm] = useState<TaxValue | null>(null)

  useEffect(() => {
    if (row) setForm(row.value as unknown as TaxValue)
  }, [row])
...
function ReceiptSection({ canWrite }: { canWrite: boolean }) {
  ...
  const [form, setForm] = useState<ReceiptValue | null>(null)

  useEffect(() => {
    if (row) setForm(row.value as unknown as ReceiptValue)
  }, [row])
...
function InventorySection({ canWrite }: { canWrite: boolean }) {
  ...
  const [form, setForm] = useState<InventoryValue | null>(null)

  useEffect(() => {
    if (row) setForm(row.value as unknown as InventoryValue)
  }, [row])
