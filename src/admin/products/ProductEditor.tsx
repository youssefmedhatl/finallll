...
export function ProductEditor() {
  ...
  }, [product, productCost, categories, brands, getLocalized])
  ...
  }, [variants, variantCosts, stockLevels])
  ...
  }, [form.name_en, isNew, handleFormChange])

  // Builds a readable, admin-editable starting-point SKU (e.g. RED-T-SHIRT-M-BLK).
  // Staff can freely overwrite this afterwards — see the SKU input in the
  // variants table below.
  const generateSkuBase = useCallback((size?: string | null, color?: string | null) => {
    const base = slugify(form.name_en || form.slug || 'item').toUpperCase().replace(/-/g, '').slice(0, 12) || 'ITEM'
    const parts = [base]
    if (size) parts.push(size.toUpperCase().replace(/\s+/g, ''))
    if (color) parts.push(color.replace(/\s+/g, '').slice(0, 3).toUpperCase())
    return parts.join('-')
  }, [form.name_en, form.slug])

  // Turns the picked sizes/colors into actual variant rows. Previously this
  // button (labelled via the generateMatrix translation key) didn't exist at
  // all, so staff had no way to create any variant — meaning new products
  // saved with zero variants, zero stock rows, and never showed up anywhere
  // stock/inventory is tracked. If no sizes or colors are picked, a single
  // "default" variant is created so even a simple product gets stock tracking.
  const handleGenerateVariants = useCallback(() => {
    const live = variantForm.variants.filter(v => !v._isDeleted)
    const existingKeys = new Set(live.map(v => `${v.size || ''}|${v.color_name || ''}`))
    const usedSkus = new Set(live.map(v => v.sku))

    type Combo = { size: string | null; color_name: string | null; color_hex: string | null }
    const combos: Combo[] = []
    if (variantForm.sizes.length === 0 && variantForm.colors.length === 0) {
      combos.push({ size: null, color_name: null, color_hex: null })
    } else if (variantForm.colors.length === 0) {
      for (const size of variantForm.sizes) combos.push({ size, color_name: null, color_hex: null })
    } else if (variantForm.sizes.length === 0) {
      for (const color of variantForm.colors) combos.push({ size: null, color_name: color.name, color_hex: color.hex })
    } else {
      for (const size of variantForm.sizes) {
        for (const color of variantForm.colors) {
          combos.push({ size, color_name: color.name, color_hex: color.hex })
        }
      }
    }

    const newRows = combos
      .filter(c => !existingKeys.has(`${c.size || ''}|${c.color_name || ''}`))
      .map((c) => {
        let sku = generateSkuBase(c.size, c.color_name)
        let n = 1
        while (usedSkus.has(sku)) {
          n += 1
          sku = `${generateSkuBase(c.size, c.color_name)}-${n}`
        }
        usedSkus.add(sku)
        return {
          ...
          _isNew: true,
          stock: 0,
        } as VariantFormState['variants'][number]
      })

    if (newRows.length === 0) {
      toast.error(t('productEditor.duplicateField', { field: 'size/color combination' }))
      return
    }

    setVariantForm(prev => ({ ...prev, variants: [...prev.variants, ...newRows] }))
    setHasChanges(true)
  }, [variantForm, generateSkuBase, t])
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      // Safety net: a product saved with zero variants gets zero stock rows
      // and then never appears anywhere stock/inventory is tracked (see
      // handleGenerateVariants above for the full explanation). If staff
      // never opened the Variants tab, auto-create one default variant here
      // so every product is always trackable.
      const hasLiveVariant = variantForm.variants.some(v => !v._isDeleted)
      const effectiveVariants = hasLiveVariant
      ...
      if (effectiveVariants.length > 0) {
        // Insert new variants
        const newVariants = effectiveVariants.filter(v => v._isNew && !v._isDeleted)
  ...
  }, [id, images.length, t, queryClient])
                        ...
                        <tr key={variant.id} className="border-b border-sand hover:bg-sand/30">
                          ...
                          <td className="py-3 px-3 font-mono text-xs" dir="ltr">
                            <input
                              ...
                              className="w-32 rounded border border-sand px-2 py-1 font-mono text-xs"
                            />
                          </td>
                          <td className="py-3 px-3 font-mono text-xs" dir="ltr">
                            {variant.barcode || '—'}
